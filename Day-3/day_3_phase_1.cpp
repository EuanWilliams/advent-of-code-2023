...
// Euan W: 03/12/2023
//
// Solution for Advent of Code day 3 Phase 1
//
...
bool check_neighbouring_number_already_counted(vector<int> neighbouring_number_ids, GridCharacter character) {
    // Checks if the character is part of a number that is in the neighbouring_number_ids list
    for (int number_id : neighbouring_number_ids) {
        if (character.number_id == number_id) {
            return true;
        }
    }
    return false;
...
tuple<vector<int>, vector<int>> check_character(GridCharacter character_to_check, vector<int> neighbouring_number_values, vector<int> neighbouring_number_ids) {
        // cout << "Checking character " << character_to_check.value;
    if (character_to_check.part_of_number && !check_neighbouring_number_already_counted(neighbouring_number_ids, character_to_check)) {
        cout << "Found number " << character_to_check.number_value << endl;
        neighbouring_number_ids.push_back(character_to_check.number_id);
        neighbouring_number_values.push_back(character_to_check.number_value);
    }
    return make_tuple(neighbouring_number_ids, neighbouring_number_values);
}

tuple<vector<int>, vector<int>> check_above_bellow_line(vector<GridCharacter> line, int character_index, vector<int> neighbouring_number_values, vector<int> neighbouring_number_ids, bool isInline) {
    ...
    if (character_index != 0) {
        ...
        tie(neighbouring_number_ids, neighbouring_number_values) = check_character(character_to_check, neighbouring_number_values, neighbouring_number_ids);
    }
...
tuple<vector<int>, vector<int>> check_for_neighbouring_numbers(vector<vector<GridCharacter>> grid, int line_index, int char_index, vector<int> neighbouring_number_values, vector<int> neighbouring_number_ids) {
    ...
    if (line_index != 0) {
        ...
        tie(neighbouring_number_ids, neighbouring_number_values) = check_above_bellow_line(line, char_index, neighbouring_number_values, neighbouring_number_ids, false);
    }

    // Check neighbouring characters
    cout << "Checking neighbouring line" << endl;
    vector<GridCharacter> line = grid[line_index];
    tie(neighbouring_number_ids, neighbouring_number_values) = check_above_bellow_line(line, char_index, neighbouring_number_values, neighbouring_number_ids, true);
...
int main()
{
    ...
    vector<int> neighbouring_number_values = {};
    vector<int> neighbouring_number_ids = {};
    for (int line_index = 0; line_index < grid.size(); line_index++) {
        for (int char_index = 0; char_index < grid[line_index].size(); char_index++) {
            ...
            if (check_symbol(character_to_test.value, false)) {
                cout << "Found symbol at (" << char_index << "," << line_index << ")" << endl; 
                tie(neighbouring_number_ids, neighbouring_number_values) = check_for_neighbouring_numbers(grid, line_index, char_index, neighbouring_number_values, neighbouring_number_ids);
            }
        }
    }
    ...
    int total = 0;
    cout << "Found ";
    for (int number : neighbouring_number_values) {
        cout << number << endl;
        total += number;
    }
    cout << endl;
